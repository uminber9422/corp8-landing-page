    ...
    <section className="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        ...
        {steps.map((step, index) => (
          <div 
            key={step.id}
            className="group relative flex flex-col bg-white border border-neutral-200 shadow-sm rounded-xl hover:shadow-md transition"
          >
            <div className="h-52 flex flex-col p-4 md:p-6 relative overflow-hidden">
              {/* Gradient Background */}
              <div className={`absolute top-0 right-0 h-full w-1/2 bg-gradient-to-l ${step.gradient}`}></div>
              
              {/* Icon */}
              <span className={`mb-5 inline-flex justify-center items-center w-12 h-12 rounded-lg ${step.color} bg-white shadow-sm`}>
                {step.icon}
              </span>
            ...
            {/* Card Footer */}
            <div className="mt-auto p-4 md:p-6 border-t border-neutral-200">
              <a className="inline-flex items-center gap-x-1 text-primary-600 hover:text-primary-800 font-medium" href="#">
                Learn more
